 ...
 * It may produce a group pdf report, or a set of user single reports
...
$id = required_param('id', PARAM_INT); // The course id from where the batch was created.
...
if ($reportlayout == 'onefulluserpersheet') {
    $reporttype = 'peruser';
    $range = 'group';
} else if ($reportlayout == 'onefulluserperfile') {
    $reporttype = 'peruser';
    $range = 'user';
} else if ($reportlayout == 'oneuserperrow') {
    $reporttype = 'summary';
    $range = 'group';
} else if ($reportlayout == 'allusersessionssinglefile') {
    $reporttype = 'sessions';
    $range = 'group';
} else {
    $reporttype = 'sessions';
    ...
    if ($range == 'user') {
        ...
        if (!empty($targetusers)) {
            ...
            foreach ($targetusers as $user) {
                ...
                $filerec->filename = "trainingsessions_user_{$user->username}_{$reporttype}_".date('Y-M-d', time()).".pdf";
        ...
        $filerec->itemid = $course->id;
        $filerec->filepath = "/{$outputdir}/{$sessionday}/";
        $filerec->filename = "trainingsessions_group_{$group->name}_{$reporttype}_".date('Y-M-d', time()).".pdf";
